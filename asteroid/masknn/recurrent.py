...
class DPRNNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, K, L, N if self.in_chan!=256 else int(N/2)).transpose(1, -1).transpose(2, -1).contiguous()
        x = self.inter_norm(x)
...
class DPRNN(nn.Module):
    ...
    def __init__(
        ...
        # self.channel_attention = SELayer(channel=128, reduction=32)
        # self.channel_attention =  MultiHeadAttention(8, 128, 16, 16)
    ...
    def forward(self, mixture_w):
        ...
        # output = output.reshape(batch, n_frames, self.bn_chan)
        # # output,att = self.channel_attention(output,output,output)
        # output = output.reshape(batch,  self.bn_chan,n_frames)
